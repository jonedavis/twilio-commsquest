...
let controllerAxis = [0, 0];
let gameState = undefined;
let BALL_RADIUS = 0.2;
let NUMBER_OF_LEVELS = 4;
let COLLISION_IMPULSE_THRESHOLD = 1.5;
let currentLevel = 0;
let yellowColor = {
  r: 231,
  g: 212,
...
let getFlashColor = utils.colors.transition(yellowColor, redColor, 0.05);
...
let $mainMenuFooter = undefined;
let $splashScreen = undefined;
let $splashScreenTitle = undefined;
let $splashScreenLevelName = undefined;
let $splashScreenLevelDetails = undefined;
let $splashScreenLevelDescription = undefined;
let $splashLevelCompleted = undefined;
let $splashLevelCompletedGraphic = undefined;
let $splashScreenGameOverMenu = undefined;
let $pauseScreen = undefined;
let $gameCanvasDisplay = undefined;
let $level = undefined;
let $levelName = undefined;
let levelNames = [
  'MAIN MENU',
  'CAVE OF LAST-CALL',
...
function createRenderWorld() {
  ...
  // Flash Plane
  const flashPlane = new THREE.PlaneGeometry(
    mazeDimension * 10,
    mazeDimension * 10,
    mazeDimension,
    mazeDimension
  ...
  const planeTexture = assets[currentLevel].concrete;
...
function updatePhysicsWorld() {
  ...
  const f = new b2Vec2(
    controllerAxis[0] * FORCE_MULTIPLIER * wBall.GetMass(),
...
function showSplashForLevel() {
  const animationDelay = currentLevel != 5 ? 3000 : 8000;
...
function onControllerUpdated(axis) {
  ...
  if (axis.value.isGamePaused) {
    $pauseScreen.show();
    return;
  } else if (!axis.value.isGamePaused && $pauseScreen.is(':visible')) {
    $pauseScreen.hide();
  }

  // Push raw data to front of arrays
  // each coordinate gets it's own array
  rawAccelX.unshift(axis.value.x);
  rawAccelY.unshift(axis.value.y);
...
function getAvgAcceleration(rawAccel) {
  let accel = 0.0;
  for (let i = 0; i < rawAccel.length; i++) {
    accel += rawAccel[i];
  }
            ...
            .then(function (ctrlDoc) {
              ...
              syncClient
                ...
                .then(function (syncList) {
                  ...
                  // Get Handle to game canvas to toggle visibility
