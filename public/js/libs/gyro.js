...
(function(root, factory) {
  ...
  function quaternionToEuler(q) {
    const s = 180 / Math.PI;
    const front = quaternionApply({ x: 0, y: 1, z: 0 }, q);
    let alpha =
    ...
    const up = quaternionApply({ x: 0, y: 0, z: 1 }, q);
    let gamma = Math.atan2(vectorDot(up, zgSide), vectorDot(up, zgUp));
      ...
      function deviceOrientationListener(e) {
        ...
        e.target.addEventListener(
          ...
          function(e) {
            ...
            const raw = eulerToQuaternion({
              alpha: e.alpha,
              beta: e.beta,
              gamma: e.gamma,
            });
            const calibrated = quaternionMultiply(calib, raw);
            const calibEuler = quaternionToEuler(calibrated);
